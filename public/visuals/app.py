...
# def get_graph_data(user_id, num_generations):
    # cursor.execute("SELECT id, name FROM people2 WHERE id = %s;", (str(user_id),))
    ...
    #         return
    #     cursor.execute("SELECT students FROM people2 WHERE id = %s;", (node_id,))
    #     students = cursor.fetchone()[0]
    #     if not students:
    #         return
    #     for student_id in students:
    #         cursor.execute("SELECT id, name FROM people2 WHERE id = %s;", (student_id,))
    ...
    #     if current_generation > num_generations:
    #         return
    #     cursor.execute("SELECT advisors FROM people2 WHERE id = %s;", (node_id,))
    #     advisors = cursor.fetchone()[0]
    #     if not advisors:
    #         return
    #     for advisor_id in advisors:
    #         cursor.execute("SELECT id, name FROM people2 WHERE id = %s;", (advisor_id,))
    ...
    try:
        ...
        def add_descendants(node_id, current_generation):
            if current_generation > num_generations:
                return
            cursor.execute("SELECT students FROM people2 WHERE id = %s;", (node_id,))
            students_str = cursor.fetchone()[0]
            if not students_str:
                return
            
            # Strip the curly braces, split by comma, and filter out any empty strings
            student_ids = [s for s in students_str.strip('{}').split(',') if s.strip()]
            ...
            for student_id in student_ids:
                try:
                    ...
                    print(f"Skipping invalid student_id: {student_id}")
        ...
        def add_advisors(node_id, current_generation):
            if current_generation > num_generations:
                return
            cursor.execute("SELECT advisors FROM people2 WHERE id = %s;", (node_id,))
            advisors_str = cursor.fetchone()[0]
            if not advisors_str:
                return
            
            # Strip the curly braces, split by comma, and filter out any empty strings
            advisor_ids = [a for a in advisors_str.strip('{}').split(',') if a.strip()]
            ...
            for advisor_id in advisor_ids:
                try:
                    ...
                    print(f"Skipping invalid advisor_id: {advisor_id}")
...
def graph():
    user_id = request.args.get('node_id')
    num_generations = int(request.args.get('num_generations'))
    data = get_graph_data(user_id, num_generations)
    if data is None:
